...
- Workflow & Proposal Automation is still not a demo UI. The `/agents/` card may be clicked, but it must show only a centered by-request holding panel — no sample/proposal UI, no draft preview, and no unrelated embedded interface until the real workflow UI is ready.
