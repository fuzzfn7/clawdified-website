...
- Local pending client-portal navigation gate: `117/117` repository tests plus `5/5` local Playwright integration checks across `/`, `/company-brain/`, and `/agent-training/`. The browser proof confirms the exact `https://app.clawdified.com` click request, desktop/390px visibility, 44px controls, no overflow, zero console errors, and the separately running local portal’s `/` → `/login` secure-link surface. This local change has not been pushed or deployed.
