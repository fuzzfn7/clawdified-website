...
- `/company-brain/` and `/agent-training/` are intentionally sparse, `noindex` Coming Soon pages that share `assets/service-coming-soon.css`. The homepage footer groups them in a dedicated `Coming soon` column, separate from the current offer and main navigation. They should remain non-indexed until real public content is ready.
...
- `/ai-agent-agency-knoxville-tn` is the one indexable Knoxville page. `/ai-agent-knoxville-tn`, `/small-business-ai-agent-knoxville-tn`, `/ai-automation-agency-knoxville-tn`, and `/business-automation-knoxville-tn` permanently redirect to it. `/about` and `/contact` are retired with empty no-store 410 responses, and a top-level `404.html` prevents Cloudflare Pages from soft-404ing unknown paths to the homepage.
- SEO/entity release gates: `140/140` repository tests, `59/59` curated-release HTTP route/content contracts, `10/10` desktop/mobile rendered-browser checks, and Lighthouse SEO `100` for the homepage and Knoxville page. Knoxville also scores `100` for Best Practices after responsive wordmark verification.
...
## Current release verification — 2026-07-26

Reviewed source and deployment contract:

- Homepage source: the Heritage Flow experience with production SEO metadata, transparent claw, pixel-exact responsive Industrial Notch wordmarks derived from the approved vector master, the black heritage tile for browser favicons, and the creamy social-profile artwork for installed-app identity.
- Reviewed release identity: `index.html` SHA-256 `af7a227e2d1fe678f86416e1c8e34491bc3e184801c073e29d3a001b66d763b0`; responsive wordmark SHA-256 values are `ff1f1aa4e2d4a52ac11af4a1e87f20faae3adf2dff49750f95d3d5ff7c0f61a9` (1×), `21335bf3507f7258b20f4004aa4e7b55d0273bd3b530b34964776520249b6424` (2×), and `d18332f2e9b27c3627f926e6ece4c6106907111df442cf68d3998cebed214120` (3×).
...
- Browser and installed-app icons are deliberately separate. Browser tabs use the black `/assets/clawdified-favicon-heritage-20260711.png` tile (SHA-256 `fa7900ee5c6b1ca2290ff4eb1f14c04b70bd6228d49d85658fa89ab01c06922f`) plus black ICO/PNG fallbacks. iOS uses the opaque creamy 180px `/clawdified-social-profile-apple-touch-icon.png`; the web manifest uses opaque creamy 192px and 512px icons with `purpose: any maskable` for Android and Samsung home-screen crops, all derived from `assets/clawdified-social-profile-final.png` (SHA-256 `c0893e3b81034759466cabc5553ebc7f688480d3ab9294ba35c918a50a448b4a`).
- Release gates: `146/146` repository tests, `59/59` curated-release HTTP contracts, `10/10` rendered desktop/mobile checks, exact staged-release hash checks, and Lighthouse SEO `100` on the homepage and Knoxville page.
...
- Manual-story pill cleanup: source commit `64fd1c2` is deployed as Cloudflare Pages production deployment `0e181461-6504-42d2-82f8-fbd29779fee0` with immutable preview `https://0e181461.clawdifiedweb.pages.dev`. The exact staged tree passed `140/140`; its curated 94-file artifact excludes repository-only roots. The preview and custom domain passed `18/18` route/status checks plus both source contracts, and all three affected 390×844 story slides rendered without the removed labels, horizontal overflow, clipping, browser errors, or loss of the four-bar stepper.
- Browser and installed-app icon refresh: source commit `46ba5f7` is deployed as Cloudflare Pages production deployment `a8bcbf05-7000-4bd1-85be-ad15753981b2` with immutable preview `https://a8bcbf05.clawdifiedweb.pages.dev`. The exact staged tree passed `145/145`; its curated 101-file artifact retains all seven canonical icon assets and excludes repository-only roots. Both the preview and custom domain served the six browser/iOS/Android/Samsung-facing files with exact committed SHA-256 bytes, correct MIME types, and immutable caching. Live Chrome decoded the multi-size ICO plus 64px, 180px, 192px, and 512px assets with zero console errors; simulated iOS, Android circular, and Samsung rounded crops preserved the creamy background, centered claw, and safe margins.
- Split browser/app icon release: source commit `fd03ee8` is deployed as Cloudflare Pages production deployment `233afdfb-96b2-4010-8274-0bc70983a803` with immutable preview `https://233afdfb.clawdifiedweb.pages.dev`; rollback is `a8bcbf05-7000-4bd1-85be-ad15753981b2`. The exact staged tree passed `146/146`; its curated 98-file artifact uses the black heritage PNG/ICO family for browser favicons while preserving the creamy Apple touch and 192px/512px maskable manifest icons. Both origins served exact committed bytes and correct cache/MIME contracts, retired white browser-only assets returned `404`, protected APIs remained `401`, and live Chrome decoded black-versus-cream pixels with zero console errors.
