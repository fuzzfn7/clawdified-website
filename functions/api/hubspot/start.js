...
export async function onRequestGet(context) {
  ...
  if (!ready.ok) {
    return json({
      ok: false,
      error: 'HubSpot connector backend is not fully configured yet.',
      missing_config: ready.missing,
      next_step: 'Set the required Cloudflare Pages secrets/bindings before sending clients through HubSpot OAuth.',
    }, 503);
  }
  ...
  const { url: authUrl, statePayload } = buildAuthUrl({
    ...
    returnTo: url.searchParams.get('return_to') || '/connect/hubspot/',
  });
