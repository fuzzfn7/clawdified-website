...
export async function onRequestGet(context) {
  ...
  if (!ready.ok) {
    return json({
      ok: false,
      error: 'Gmail connector backend is not fully configured yet.',
      missing_config: ready.missing,
      next_step: 'Set the required Cloudflare Pages secrets/bindings before sending clients through Google OAuth.',
    }, 503);
  }
