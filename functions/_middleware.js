...
const LEGAL_ROUTES = {
  '/privacy': {
    ...
    description: 'Clawdified privacy policy for the website and mobile app, client workflows, voice notes, and authorized third-party connectors.',
    eyebrow: 'Privacy Policy',
    updated: 'August 2, 2026',
    sections: [
      ['Overview', 'This policy covers the Clawdified website and mobile app. It explains what information we collect, how we use it to provide authorized client workflows, and how users can request access changes or deletion.'],
      ['Account and workspace information', 'When you sign in to the mobile app, we process your email address, display name, authorized organization and workspace identifiers, assigned agent, and capability labels. The app uses a secure session cookie and a request-verification token to keep the signed-in workspace scoped to the correct user; it does not store your password after sign-in.'],
      ['Work Order data', 'The app displays Work Order data that the authorized client workspace makes available, including record identifiers, work-order numbers, customer and location details, job summaries, workflow status, source-version information, and authorized note metadata. We use this data only to provide and protect the requested workflow.'],
      ['Voice recordings and transcripts', 'The mobile app collects voice recordings and transcripts only when you choose to record a note for a selected Work Order. Microphone access is requested only when you tap the microphone to begin recording. The app does not listen in the background and does not use unrestricted wake-word monitoring. Recordings are uploaded to the authorized workspace for transcription and workflow use.'],
      ['Other information we collect', 'We may collect business contact details, project notes, workflow requirements, messages you send to us, website usage information, and information you explicitly authorize through third-party integrations such as Meta/Facebook Login.'],
      ...
      ['How we use information', 'We use information to authenticate users, show the correct client workspace, provide Work Order and voice-note features, transcribe authorized recordings, secure and troubleshoot the service, communicate with clients, improve reliability, and comply with platform or legal requirements. We do not sell personal information and do not use app data for advertising or cross-app tracking.'],
      ['Sharing and service providers', 'We share information only with service providers needed to host, secure, transcribe, or operate the authorized workflow; with a third-party tool when the client directs us to connect it; or when required by law. Service providers may process data only for the functions they perform for Clawdified or the authorized client workflow.'],
      ['Retention and security', 'Information is retained while needed to provide the authorized workflow and for applicable legal, security, billing, backup, or dispute-resolution needs. Retention can also depend on the client agreement and connected source system. We use reasonable technical and organizational safeguards, but no internet service can guarantee absolute security.'],
      ['Your choices and deletion', 'Where the workflow allows it, an authorized user can delete an individual voice note in the app before the Work Order is marked Ready. You can sign out to end the local app session, revoke third-party access through the connected platform, or request deletion through https://clawdified.com/data-deletion. Some limited records may be retained where legally or operationally required as described on that page.'],
